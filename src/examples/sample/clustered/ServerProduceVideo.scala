    ...
    case ActorIdentity(correlationId, optConsumerActorRef) =>
      optConsumerActorRef.foreach {
        consumerActorRef =>
          ...
          val fileProducer: Publisher[Frame] = video.FFMpeg.readFile(mp4, context.system)
          val consumer = ActorSubscriber[Frame](consumerActorRef)
          fileProducer.subscribe(consumer)
      }
